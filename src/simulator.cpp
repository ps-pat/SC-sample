...
void Simulator::printUsage() {
    cout<<"Usage: <samplesize> <region in base pairs> <-f filename> [options]"<<endl;
    cout<<"Options: "<<endl;
    ...
    cout<<"-f <input sample file> sample file name, note that the sample size in given sample file must be same as the value set by <samplesize>"<<endl;
    ...
    for (unsigned int iCurrentArg = 1;iCurrentArg<iTotalArgs;++iCurrentArg){
        try{
            ...
            switch (arguments[iCurrentArg][0][1] ){
                ...
                case 'f':
                    if (arguments[iCurrentArg].size()!=2){
                        cerr<<"sample filename is not given!"<<endl;
                        exit(1);
                    }
                    pConfig->fsample = arguments[iCurrentArg][1].data(); 
                    cerr<<"INPUT: Sample file is "<<pConfig->fsample<<endl;
                    break;
                case 'F':
...
void Simulator::beginSimulation(){
    try{
        ...
        for(unsigned int i=0;i<pConfig->iIterations;++i){
            ...
            GraphBuilder graphBuilder = GraphBuilder(pConfig,rg,&samples);
            graphBuilder.build();
...
int main(int argc,char * argv[]){
    ...
    try{
        ...
        if(argc<3){
            ...
            "the number of chromosomes sampled , the sequence length and the sample file name."<<endl;
